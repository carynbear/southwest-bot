...
class Alert {
  ...
  constructor (originalData) {
    ...
    this.data.passengerCount = Number.isInteger(this.data.passengerCount) ? this.data.passengerCount : 1;

    this.data.phone = this.data.phone ? this.data.phone.split('').filter(d => /\d/.test(d)).join('') : null;
    this.data.toEmail = this.data.toEmail ? this.data.toEmail.split('').filter(d => /\S/.test(d)).join('') : this.data.user.split('').filter(d => /\S/.test(d)).join('');
    this.data.toDiscord = this.data.toDiscord ? this.data.toDiscord.split('').filter(d => /\S/.test(d)).join('') : null;
  ...
  get passengerCount () { return this.data.passengerCount; }
  ...
  get toDiscord () { return this.data.toDiscord; }
  get priceHistory () { return this.data.priceHistory; }
  get alertType () { return this.data.alertType; }
  get lastChecked () { 
    const date = new Date(Math.max(...this.data.priceHistory.map(d => d.time)))
    return dateFormat(date, 'm/d h:ss TT Z')
  }
